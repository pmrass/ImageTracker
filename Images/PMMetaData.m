classdef PMMetaData
    %PMMETADATA Summary of this class goes here
    ...
    methods
        ...
        function obj = PMMetaData(varargin)
            ...
            switch NumberOfArguments
                ...
                case 9
                    ...
                    for FrameIndex=1:length(Value{9})
                        CurrentFrame=                                       Value{9}(FrameIndex);
                        myStructure.TimeStamp(FrameIndex,1)=                CurrentFrame;
                        myStructure.RelativeTimeStamp(FrameIndex,1)=        CurrentFrame- Value{9}(1);
                    end
              ...
              if isstruct(Value)
                    ExpectedFieldNames =     {'NumberOfRows';'NumberOfColumns';'NumberOfPlanes';'NumberOfTimePoints';'NumberOfChannels';'VoxelSizeX';'VoxelSizeY';'VoxelSizeZ'};
                     ExpectedFieldNamesTwo = {'NumberOfRows';'NumberOfColumns';'NumberOfPlanes';'NumberOfChannels';'VoxelSizeX';'VoxelSizeY';'VoxelSizeZ';'NumberOfTimePoints'};
                       ...
                       if isequal(ExpectedContentsForIndividual, MyFieldNames)
                           if isstruct(Value.EntireMovie) && isnumeric(Value.TimeStamp) && isnumeric(Value.RelativeTimeStamp)

                               ActualFieldNames =       fieldnames(Value.EntireMovie);
                               if isequal(ExpectedFieldNames, ActualFieldNames)
                                    type = 'Individual';
                               elseif isequal(ExpectedFieldNamesTwo, ActualFieldNames)
                                    type = 'IndividualTwo';
                               else
                                   obj.compareFieldNames(ExpectedFieldNames, ActualFieldNames)
                                   error('Meta-data fieldnames do not match.')
                               end
                           end
                       elseif isequal(ExpectedContentsForPooled, MyFieldNames)
                           if isstruct(Value.EntireMovie) && isnumeric(Value.PooledTimeStamps) && isnumeric(Value.RelativeTimeStamps)
                               ExpectedFieldNames =     {'NumberOfRows';'NumberOfColumns';'NumberOfPlanes';'NumberOfTimePoints';'NumberOfChannels';'VoxelSizeX';'VoxelSizeY';'VoxelSizeZ'};
                               ActualFieldNames =       fieldnames(Value.EntireMovie);
                               if isequal(ExpectedFieldNames, ActualFieldNames)
                                    type = 'Pooled';
         ...
         function compareFieldNames(obj, one, two)
        ...
        function result = convertPooledToIndividualMetaMata(obj,Value)
