classdef PMSegmentationCapture
    ...
    methods % SETTERS: set multiple properties by MovieController
        ...
        function obj=               resetWithMovieTracking(obj, MovieTracking)
            % RESETWITHMOVIECONTROLLER set state with MovieController;
            % takes 1 argument:
            % 1: PMMovieController
            ...
            obj =        obj.setImageVolume(MovieTracking.getActiveImage);
    ...
    methods % SETTERS AUTOCELLRECOGNITION BY INTENSITY
        ...
        function obj =      setActiveCoordinateByBrightestArea(obj)
            % SETACTIVECOORDINATEBYBRIGHTESTAREA finds brightest image area (corresponding to SizeForFindingCellsByIntensity) of active plane of image-volume;
            % then sets active X and Y coordinate by the detected point;
            % blacked-out pixels are ignored;
            
            obj =                               obj.resetMask;
            BlackedOutImage =                   obj.getImageWithBlackedOutPixelsForPlane(obj.ActiveZCoordinate);
            ...
            [xCoordinate, yCoordinate] =    PMImage(FilteredImage).getBrightestPositionInImage;
    ...
    methods % SETTERS BLACKED-OUT PIXELS
        ...
        function obj =      addActivePixelsToBlackedOutPixels(obj)
            ...
            Center =                round([obj.getXCentroid, obj.getYCentroid, obj.getZCentroid]);
            Coordinates =           PMShape().getCoordinatesAroundCenter(Center, obj.getMinimumCellRadius);
            obj =                   obj.addPixelsToBlackedOutPixels(Coordinates);
    ...
    methods % SETTERS SEGMENTATION
        ...
        function obj =                  generateMaskByAutoThreshold(obj)
            ...
            CroppedImage =               obj.getCroppedImageWithBlackedOutPixelsRemoved; % get cropped image where all pixels from previously tracked cells are set to 0;
            Threshold =                  obj.getThresholdFromEdge;
            ThresholdedImage =           uint8(PMImage(CroppedImage).threshold(Threshold).getImage);
    ...
    methods (Access = private) % GETTERS CROPPED IMAGE
         ...
         function empytImage =       getCroppedImageSource(obj)
             % GETCROPPEDIMAGESOURCE returns cropped image;
             % cropped image is centered around active coordinate +/- MaximumDisplacement;
             % minimum row/column = 1;
             % maximum row/column = size of image;
             % planes that are beyond plane limits are blacked out;
                image =                     obj.getCropImageObjectForImage(obj.ImageVolume).getImage; % get cropped image
                empytImage =                image;
                empytImage(:, :, :) =       0;
              ...
              %  currentAxesOne= subplot(3, 3, 8);
               % currentAxesOne.Visible = 'on';
                ...
                title('After shape verification')
                 ...
                 Text.Color = 'w';
                 obj.getActiveShape.testShapeValidity
    ...
    methods (Access = private) % GETTERS CROPPED IMAGE FOR SEGMENTATION;
          ...
          function Image =                          getCroppedImageWithExistingMasksRemoved(obj)
            ...
            CoordinatesToZeroOut =           obj.removeCroppingFromCoordinateList(obj.getAllPreviouslyTrackedPixels);
             ...
             Image =                     PMImage(obj.getCroppedImageSource).addCoordinatesWithIntensity(CoordinatesToZeroOut, 0).getImage;
    ...
    methods (Access = private) % PIXEL INTENSITIES:
