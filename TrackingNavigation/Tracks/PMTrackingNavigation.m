classdef PMTrackingNavigation
    ...
    methods % INITIALIZATION
        ...
        function obj = set.TrackingCellForTimeWithDrift(obj, Value)
            ...
            try
               assert( size(Value, 2) == 0 || size(Value, 2) == 1, 'Wrong input.');
            catch
    ...
    methods % SETTERS FILE-MANAGEMENT
        ...
        function obj =      loadMasks(obj)
            ...
            try
                ...
                tic
                load(obj.getFileNameForStoringMaskData, 'MaskPixels')
                for timeIndex = 1 : length(MaskPixels)
                      obj.TrackingCellForTime{timeIndex, 1}(:, obj.PixelColumn) = MaskPixels{timeIndex};
                end
                fprintf('Loading of mask data: %6.2f seconds.\n', toc)
    ...
    methods % GETTERS: TRACKINGCELLFORTIME- TRACK-IDs:
        ...
        function newTrackID =           getNewTrackID(obj, varargin)
            ...
            assert(PMNumbers(Number).isNoNanVector, 'Wrong input.')
    ...
    methods % GETTERS SEGMENTATION LIST
        ...
        function Coordinates =          getSelectedMasksAtFramePlaneDrift(obj, Frame, Plane, Drift)
            ...
            % 2: wanted plane numbers: this is only for filtering pixels; they are always without drift, therefore this value must be without drift;
            % 3: PMDriftCorrection object
    ...
    methods % SETTERS TRACKINFOLIST
        ...
        function obj = setTrackInfoListFromScratch(obj)
              % SETTRACKINFOLISTFROMSCRATCH cleans up tracking-info and segmentation addresses;
            % orders tracks in info list, adds missing IDs to info-list, resets segmentation addresses for all tracked information;
            tic
            fprintf('Set tracking info from scratch: \n')
             ...
             ListWithTrackIDs =     (obj.getUniqueTrackIDsFromTrackingCell);
             
              obj.SelectedTrackIDs =            ListWithTrackIDs';
       
             
         
            obj.TrackInfoList =       arrayfun(@(x) PMTrackInfo(x), (1 : max(ListWithTrackIDs))', 'UniformOutput', false);

            
            
            for TrackID =  ListWithTrackIDs'
                tic
                fprintf('TrackID %i: ', TrackID)
        ...
        function obj =      generalCleanup(obj)
          error('Use setTrackInfoListFromScratch instead.')
    ...
    methods % SETTERS TRACKINGCELLFORTIME: 
        ...
        function obj =                      removeTracksWithFramesLessThan(obj, FrameNumberLimit)
            ...
            fprintf('\nDeleting tracks with %i or less frames:\nDeleting track ', FrameNumberLimit)  
             ...
             MyTrackIDs =                           cell2mat(pooled(:, obj.TrackIDColumn));
             MyUniqueTrackIDs =                     unique(MyTrackIDs);
             
             TrackLengths =                         arrayfun(@(x) sum(x == MyTrackIDs), MyUniqueTrackIDs);
             SelectedRows =                         TrackLengths <= FrameNumberLimit;
             MyUniqueTrackIDs(~SelectedRows) =      [];
             
            pooled =                               obj.removeFromPooledMatrixTrackIDs(pooled, MyUniqueTrackIDs);
            pooledWithDrift =                      obj.removeFromPooledMatrixTrackIDs(pooledWithDrift, MyUniqueTrackIDs);
            
            
              MyTrackIDs =                           cell2mat(pooled(:, obj.TrackIDColumn));
             MyUniqueTrackIDs =                     unique(MyTrackIDs);
            
             MyNewUniqueTrackIDs =              (1 : length(MyUniqueTrackIDs))';
            
             for index =  1 : length(MyUniqueTrackIDs)
                 
                 RowsToChange = MyTrackIDs == MyUniqueTrackIDs(index);
                 
                 pooled(RowsToChange, obj.TrackIDColumn) = {MyNewUniqueTrackIDs(index)};
                 pooledWithDrift(RowsToChange, obj.TrackIDColumn) = {MyNewUniqueTrackIDs(index)};
    ...
    methods (Access = private)
        ...
        function obj =                      applyCoordinatesToTrackingCellForTimeWithDrift(obj, coordinates)
                ...
                for index = 1 : length(coordinates)
                     ...
                     obj.TrackingCellForTimeWithDrift{index, 1}(:, obj.CentroidYColumn) = num2cell(coordinates{index}(:,1));
                     obj.TrackingCellForTimeWithDrift{index, 1}(:, obj.CentroidXColumn) = num2cell(coordinates{index}(:,2));
                     obj.TrackingCellForTimeWithDrift{index, 1}(:, obj.CentroidZColumn) = num2cell(coordinates{index}(:,3));
    ...
    methods (Access = private)  % SETTERS TRACKINGCELLFORMTIME: SETTRACKFORFRAMETRACKIDWITHMASKS;
        
        function obj =      setTrackForFrameTrackIDWithMasks(obj, FrameNumber, TrackID, TargetColumns, varargin)
            ...
            TargetRow =      obj.getTargetRowForFrameTrackID(FrameNumber, TrackID);
            obj =            obj.setTrackSegmentationForFrameRowColumn(FrameNumber, TargetRow, TargetColumns, varargin{:});

        end
        
        function obj =      setTrackSegmentationForFrameRowColumn(obj, Frame, Rows, Columns, varargin)
            assert(PMNumbers(Frame).isNoNanScalar, 'Wrong input.')
            assert(PMNumbers(Rows).isNanScalar || PMNumbers(Rows).isNoNanVector, 'Wrong input.')
            assert(PMNumbers(Columns).isNanScalar || PMNumbers(Columns).isNoNanVector, 'Wrong input.')
            ...
            if isnan(Columns)
              ...
              assert(PMNumbers(Columns).isNoNanScalar, 'Currently not supported.')
    ...
    methods (Access = private) % GETTERS SEGMENTATION-LIST
        ...
        function [pooledData, pooledDataWithDrift] =                   getPooledTrackingData(obj)
            ...
            if isempty(obj.TrackingCellForTimeWithDrift)
                pooledDataWithDrift = pooledData;
            else
        ...
        function [TrackingCell, WithDrift] = getTrackingCellForTimeForFrames(obj, Frames)
             ...
             assert(PMNumbers(Frames).isNanScalar || PMNumbers(Frames).isNoNanVector, 'Wrong input.')
        ...
        function Segmentation =             getTrackingSegmentationForFrameRowColumnPropertyName(obj, Frame, Row, Column, Property)
            ...
            assert(PMNumbers(Frame).isNoNanScalar, 'Wrong input.')
            assert(PMNumbers(Row).isNanScalar || PMNumbers(Row).isNoNanVector, 'Wrong input.')
            assert(PMNumbers(Column).isNanScalar || PMNumbers(Column).isNoNanVector, 'Wrong input.')
        ...
        function SegmentationList =             getSegmentationWithAppliedDriftForFrame(obj, Frame, DriftCorrection)

             if DriftCorrection.getDriftCorrectionActive
                 SegmentationCellWithNoDrift =                  obj.getTrackingSegmentationForFrameRowColumn(Frame, NaN, NaN);
                 ...
                 SegmentationList =      obj.getTrackingSegmentationForFrameRowColumn(Frame, NaN, NaN);
    ...
    methods (Access = private) % getters track-related
        ...
        function Tracks = getUniqueTrackIDsFromTrackingCell(obj)
          ...
          function Tracks = getUniqueFramesFromTrackingCell(obj)
            Tracks =                        unique(obj.extractFramesFromTrackList(obj.getPooledTrackingData));
            Tracks(isnan(Tracks), :) =      [];
        end
    ...
    methods (Access = private) % set active track
       ...
       function obj = ActiveTrackWasChanged(obj)
        ...
        function TrackIds = removeTrackIdsThatDoNotExist(obj, TrackIds)
            ...
            ExistingTracks = obj.getListWithAllUniqueTrackIDs;
            
            if isempty(ExistingTracks)
                
                
            else
                  Rows =             ismember(TrackIds, ExistingTracks);
                    MissingRows =   ~Rows;
    ...
    methods (Access = private) % GETTERS TRACKINGMATRIX getCoordinatesForTrackIDsInternal
        ...
        function coordinateList =       getListWithAllCoordinates(obj)
           ...
           coordinateList = arrayfun(@(x) obj.extractCoordinatesFromTrackList(obj.getTrackingCellForTimeForFrames(x)), ...
               (1 : obj.getMaxFrame)', ...
               'UniformOutput', false); 
        end

        function coordinateList =       getListWithAllDriftCorrectedCoordinates(obj)
          coordinateList = arrayfun(@(x) obj.extractCoordinatesFromTrackList(obj.TrackingCellForTimeWithDrift{x}), ...
              1 : obj.getMaxFrame, ...
              'UniformOutput', false); 
        end
