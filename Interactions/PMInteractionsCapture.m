classdef PMInteractionsCapture
    ...
    methods % summmary
     ...
     methods % complicated setters
        ...
        function obj = setWith(obj, Value) % this is complciated; maybe get rid of it?;
            
            switch class(Value)
                
                case 'PMMovieTracking'
                
                   switch class(Value.getInteractions)
                       case 'PMInteractionsCapture'
                            obj = Value.getInteractions; % this seems strange
                       otherwise
                           error('Movie tracking input did not have PMInteractionsCapture')
                   end

                   if isempty(obj.ThresholdsForImageVolumes)
                        obj = obj.setThresholdsForImageVolumeOfTarget(Value.getDefaultThresholdsForAllPlanes);
                   end
                   
                   
                case 'PMInteractionsManager'   
                    obj = obj.set(Value.getPMInteractionsQuantification.getContent{:}); % this seems also strange; check if this is necessary;
    ...
    methods (Access = private) % file export
    ...
    methods % create interactions object:
      ...
      methods (Access = private) % shapes of "targets"
        ...
        function myFluShape_Pixels = getPixelFluShape(obj)
          ...
          function  FluVolumeOfCurrentFrame = getImageVolumeOfTarget(obj)
               obj =                            obj.setChannelProperties; % probably redundant because called when initiation object;
                FluVolumeOfCurrentFrame =       obj.ActiveMovieController.getActiveImageVolumeForChannel(obj.ChannelNumbersForTarget);
               
          end
          
          function obj = setChannelProperties(obj)
              if isempty(obj.ActiveMovieController)
                  warning('Currently no active movie controller set. Therefore only limited functionality.')
              else
                    ...
                    obj.ActiveMovieController =     obj.ActiveMovieController.setLoadedMovie(MyLoadedMovie);
