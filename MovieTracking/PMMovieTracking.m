classdef PMMovieTracking < PMChannels
    ...
    methods % SETTERS IMAGE-VOLUMES
        ...
        function obj =                      updateLoadedImageVolumes(obj, varargin)
            ...
            switch length(varargin)
                ...
                case 0
                        WantedFrames(1: obj.getMaxFrame, 1) = false;
                                         WantedFrames(obj.getActiveFrames) = true;
    ...
    methods % GETTERS LOADED IMAGE-VOLUMES: DERIVATIVE
        ...
        function Final = getActiveProcessedImageVolume(obj, varargin)
            
            imageVolumeWithFilteredInt =                obj.getActiveImageVolumeWithProcessedIntensity(varargin{:});
            Final =                                     imageVolumeWithFilteredInt(:, :, :, obj.getActiveChannelIndex);
        ...
        function activeVolume =         getActiveImageVolume(obj, varargin)
            % GETACTIVEIMAGEVOLUME returns active image volume
            % takes 0 or 1 arguments:
            % 1: index of wanted time frame (default: active frame)
            ...
            switch length(varargin)
             ...
             obj =                  obj.updateLoadedImageVolumes(Frame);
    ...
    methods % CONVERSION: IMAGE-VOLUMES:
        ...
        function rgbImage =             convertImageVolumeIntoRgbImage(obj, SourceImageVolume)
            ...
            % takes 1 argument:
            % 1: 4D numerical array: rows, columns, planes, channels
            % returns 3D numerical array: rows, columns, rgb colors
            % makes maximum-projection of all active planes;
            % apply visibility, color, min, max of each source-channel;

                ImageVolume_Source =        obj.filterImageVolumeByActivePlanes(SourceImageVolume);
                rgbImageObject =            obj.getRgbImageObjectForVolume(ImageVolume_Source);
                imageVolume =               rgbImageObject.getImageVolumeWithAdjustedIntensities;
                rgbImage =                  rgbImageObject.getRgbImageFromImageVolume(imageVolume);
    ...
    methods % SETTERS TRACKING MANUAL TRACKING
        ...
        function obj =      performTrackingMethod(obj, Value, varargin)
            ...
            if ismethod(obj.Tracking.getModel, Value)
     ...
     methods % SETTERS: AUTOCELLRECOGNITION
        ...
        function obj =      initializeAutoCellRecognition(obj)
        ...
        function obj = setAutoCellRecognition(obj, Value)
           obj.AutoCellRecognition = Value;
        end
    ...
    methods % AUTOCELLRECOGNITION: ACTION
        ...
        function SegmentationCaptureSourceList =        getSegmenationCaptureSourceListForFramesChannel(obj, FramesThatShouldBeTracked, ChannelIWant)
            ...
            ListWithImageVolumes =              arrayfun(@(x) obj.getActiveImage(x), FramesThatShouldBeTracked, 'UniformOutput', false);
            SegmentationCaptureSourceList =     cellfun(@(segmentation, x)          mySegmentationCaptureSource.setImageVolume(x), SegmentationCaptureSourceList, ListWithImageVolumes); 
    ...
    methods % SETTERS TRACKING AUTO-TRACKING
        ...
        function obj =      trackByMinimizingDistances(obj)
            
           obj.Tracking = obj.Tracking.performMethod('trackByMinimizingDistancesOfTracks',  obj.DriftCorrection, obj.SpaceCalibration, obj.TimeCalibration);
        end
    ...
    methods % GETTERS TRACKING: TRACK-IDS
        ...
        function TrackIDsToShow =       getIDsOfAllVisibleTracks(obj)
            ...
            TrackIDsToShow = unique(TrackIDsToShow);
        end
    ...
    methods (Access = private) % PROCESSING: IMAGES
       
        
         
        function CleanedUpVolume = blackOutOutOfRangePixelsInMatrix(obj, MyImageMatrix)
            
             CleanedUpVolume =               MyImageMatrix;
            CleanedUpVolume(:, :) =         0;
              ...
              try
                ...
                if isscalar(CleanedUpVolume)
                    ...
                    MyImageMatrix(Rectangle(2): Rectangle(2) + Rectangle(4)-1,Rectangle(1): Rectangle(1) + Rectangle(3) - 1);
                end
        ...
        function rgbImageObject = getRgbImageObjectForVolume(obj, ImageVolume_Source)
            
               rgbImageObject =    PMRGBImage(...
                                            ImageVolume_Source, ...
                                            obj.getIndicesOfVisibleChannels, ...
                                            obj.getIntensityLowOfAllChannels, ...
                                            obj.getIntensityHighOfAllChannels, ...
